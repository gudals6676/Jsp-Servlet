...
public class BoardDAO {
	...
	private static Connection getConnect() throws Exception { //connection pool
	...
	public int boardDelete(int idx) throws Exception{
		...
		ps = conn.prepareStatement(SQL);//전송 객체를 만듬
		ps.setInt(1, idx);
		int cnt = ps.executeUpdate();//실행 성공여부를 int 형으로 받음 
		return cnt;
	}	
	//boardContent()
	public BoardVO boardContent(int idx) throws Exception{
		Connection conn = getConnect();                                                                       //공백 문자 띄움
		String SQL ="select idx,title,count, contents,writer, DATE_FORMAT(indate,'%Y-%m-%d') AS indate from tbl_board " + 
				"where idx = ?";
		ps = conn.prepareStatement(SQL);//전송 객체를 만듬
		ps.setInt(1, idx);
